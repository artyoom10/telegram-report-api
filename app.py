    ...
    <html>
      <head>
        ...
        <style>
          ...
          .kpi-grid {{ width: 100%; border-collapse: separate; border-spacing: 10px; margin: 6px -10px 14px -10px; }}
          .kpi {{ border: 1px solid #e5e5e5; border-radius: 12px; padding: 10px 12px; background: #fafafa; vertical-align: top; }}
          ...
          table.data thead th {{ text-align: left; font-size: 11px; color: #444; border-bottom: 1px solid #ddd; padding: 8px 6px; }}
          table.data tbody td {{ border-bottom: 1px solid #eee; padding: 8px 6px; font-size: 11px; }}
      ...
      <body>
        ...
        <table class="kpi-grid">
          <tr>
            <td class="kpi"><div class="t">Всего уязвимостей</div><div class="v">{total}</div><div class="h">Все статусы</div></td>
            <td class="kpi"><div class="t">Открытые</div><div class="v">{open_cnt}</div><div class="h">Статус: Открыто</div></td>
            <td class="kpi"><div class="t">Open с CVSS ≥ 7</div><div class="v">{open_high7}</div><div class="h">Высокий и выше</div></td>
            <td class="kpi"><div class="t">Хосты</div><div class="v">{hosts}</div><div class="h">Хосты с open: {hosts_open}</div></td>
          </tr>
        </table>
        ...
        <div class="muted" style="margin-top:10px;">Сформировано автоматически.</div>
      </body>
    </html>
    """
...
def send_report():
    ...
    # Новый формат (структура)
    department = payload.get("department")
    kpis = payload.get("kpis")
    rows = payload.get("open_rows")

    # Старый формат (title/text) — поддержка, чтобы не было unknown, если фронт ещё не обновился
    if not department and ("title" in payload or "text" in payload):
        department = payload.get("department") or payload.get("scope") or payload.get("scopeLabel") or "all"
        title = str(payload.get("title") or f"Отчёт {today} по отделу {department}")
        text = str(payload.get("text") or "")

        html_doc = build_report_html(
            title=title,
            department=str(department),
            kpis={},          # KPI нет в старом формате
            rows=[],          # таблицы нет в старом формате
            generated_date=today
        )
        pdf_bytes = HTML(string=html_doc).write_pdf()
        filename = f"{_safe_filename(str(department))}_{today}.pdf"
        caption = title
        send_pdf(CHAT_ID, pdf_bytes, filename=filename, caption=caption)
        return _cors(jsonify({"ok": True, "mode": "legacy"}))

    # Новый формат нормальный
    department = str(department or "unknown")
    ...
    send_pdf(CHAT_ID, pdf_bytes, filename=filename, caption=caption)
    return _cors(jsonify({"ok": True, "mode": "structured"}))
